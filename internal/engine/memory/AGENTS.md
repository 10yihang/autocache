...
## OVERVIEW
This package is the performance-critical in-memory engine: sharded storage, expiry, eviction, Redis-type objects, unsafe helpers, and slot-aware key tracking live here.

## WHERE TO LOOK
- `internal/engine/memory/store.go` - public store facade, TTL/object coordination, stats, slot tracking
...
- `internal/engine/memory/*_test.go` - behavior, slot, unsafe, and scan coverage
...
- Preserve Redis wrong-type, integer, and not-found semantics through `pkg/errors` sentinels.
- Performance tweaks belong with benchmarks or targeted tests when they affect hot structures like shards, ring buffers, dicts, or unsafe helpers.
